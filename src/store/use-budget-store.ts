...
const initialState: Pick<BudgetStore, "settings" | "plans" | "presets" | "scenarios"> = {
    ...
    (set, get) => ({
      ...
      setActivePlan: (planId) =>
        set((state) => ({
          settings: {
            ...state.settings,
            activePlanId: state.plans[planId] ? planId : state.settings.activePlanId,
          },
        })),
      toggleRealityCheck: () =>
        set((state) => ({
      ...
      deletePlan: (planId) =>
        set((state) => {
          ...
          const nextActive =
            state.settings.activePlanId === planId ? remainingIds[0] : state.settings.activePlanId
          return {
            plans: rest,
            settings: {
              ...state.settings,
      ...
      upsertCategory: (planId, category) =>
        get().updatePlan(planId, (plan) => {
          const index = plan.categories.findIndex((item) => item.id === category.id)
      ...
      removeCategory: (planId, categoryId) =>
        get().updatePlan(planId, (plan) => ({
          ...plan,
          categories: plan.categories.filter((category) => category.id !== categoryId),
        })),
      ...
      updateScenario: (scenarioId, updater) =>
        set((state) => {
          ...
          const updated = updater({ ...scenario, plan: clonePlan(scenario.plan) })
