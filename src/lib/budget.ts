...
export function sinkingMonthly(target: number, isoDueDate: string, now = new Date()) {
...
export type PlanWarningType = "overspend" | "negative-leftover" | "underfunded-goals"
...
export function calculateIncomeMonthly(plan: Plan) {
  ...
  const extras = plan.income.extras
    ?.map((income) => normaliseToMonthly(income.amount, income.frequency))
    .reduce((sum, value) => sum + value, 0) ?? 0
  return base + extras
}

export function sumByBucket(plan: Plan, bucket: BudgetBucket, now = new Date()) {
  return plan.categories
    .filter((category) => category.bucket === bucket)
    .reduce((total, category) => total + resolveCategoryAmount(category, now), 0)
}

export function calculatePlanSummary(plan: Plan, now = new Date()): PlanSummary {
  ...
  const debt = plan.debts.reduce((total, debtItem) => total + debtItem.minimum, 0)
...
export function evaluatePlanWarnings(plan: Plan, now = new Date()): PlanWarning[] {
  const summary = calculatePlanSummary(plan, now)
  const warnings: PlanWarning[] = []

  if (summary.needs + summary.wants + summary.savings + summary.debt > summary.income) {
    warnings.push({
      type: "overspend",
  ...
  if (summary.leftover < 0) {
    warnings.push({
      ...
      message: "Your plan leaves no room for leftover cash. Adjust allocations until leftover is zero or positive.",
    })
  }
  ...
  if (monthlyGoalNeed > 0 && monthlyGoalNeed > summary.savings) {
    warnings.push({
      ...
      message: "Savings goals need more monthly allocation to hit targets on time.",
    })
  }
